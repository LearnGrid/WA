# Slides -- Building and Editing Decks

Build-time reference for slide deck artifacts: the artifact contract, the workspace export contract, and the design rules every deck must follow. This file is injected automatically when a slides artifact is created. When editing an existing deck in a later session, read it before writing any slide files.
...
Before declaring the deck done, read `./visual-qa.md` and walk every check -- it covers `/allslides` rendering, 16:9 bounds, text-scaling integrity, and the final visual QA loop.
</context>
...
**Minimum readable size at export.** `1.5vw` is the absolute floor -- captions and footnotes only. Body copy targets `2vw+` (aim for 2-2.5vw); nothing on the slide should render below `1.5vw`. Read the smallest font size across your slide JSX; if any body text falls below `2vw`, or anything at all falls below `1.5vw`, raise it. See `./visual-qa.md` for the full text-scaling rules and forbidden patterns (no `clamp()` with px caps, no `transform: scale` on text, no hardcoded `px` font sizes).
...
7. **Read `./visual-qa.md` and run every check** -- walk through your slide JSX file by file, apply the code-level checks, and fix any clipping or scaling issues.
...
The deeper text-scaling forbidden patterns (`clamp` with px caps, `transform: scale` on text, container queries, etc.) and the `/allslides` final verification loop live in `./visual-qa.md` -- **read it before finishing the deck.**
