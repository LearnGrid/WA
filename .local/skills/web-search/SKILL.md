...
### webSearch({ query, searchQueries?, count?, includeDomains?, isTextExtended? })
...
- `includeDomains` (list of str, optional): Limit to specific sites
- `isTextExtended` (bool, optional): Increase returned text from 300 to 800 characters. Use when looking for high-pri info.
- `category` (str, optional): Ignored — omit it.

**Returns:** Dict with `searchAnswer` and `resultPages` (list of title/url/snippet dicts). Whenever you pass any argument beyond `query`, `searchQueries` and `count`, each result also carries `publishedDate` when the search engine has it. There are no structured `entities` records.
...
- You can search social media (LinkedIn, Instagram, Facebook, Reddit, YouTube), but you can't open the pages via `webFetch`.
